...
MCPGuard is a security tool, so its own posture and boundaries must be explicit
and auditable. This document states what MCPGuard does and does not do, how to
report a vulnerability, and the guarantees you may and may not rely on.

## What MCPGuard is

MCPGuard is a **deterministic, offline, static** pre-flight scanner for MCP
...
- **Never reads real secrets.** MCPGuard inspects configuration shape, not
...
## What MCPGuard does NOT guarantee
...
- MCPGuard does not observe runtime behavior (what a server reads, writes,
  spawns, or sends when actually run). Runtime/deep-scan analysis is out of
  scope for this release.
- MCPGuard is not a gateway, proxy, marketplace, or policy-enforcement runtime.
...
If you believe you have found a security issue in MCPGuard itself — for example
...
MCPGuard is pre-1.0 and under active hardening. Security fixes target the latest
