# Gate S1 鈥?carried open items, in committed bytes
...
slice** on the expansion ladder `S0(25) 鈫?S1(100) 鈫?S2(500) 鈫?S3(all) 鈫?S4(second source)`,
...
measures S1 is supposed to take are specified at `docs/new15-execution-status.md:342` 鈥?a file
...
**19 鈫?100 鈫?150** under ADR 0086's auto-growth and passed S1's 100-record threshold with
...
all** 鈥?the limit case, and the harder one to notice, because a missing guard has no green to
...
## First measurement 鈥?2026-08-28, at `fce13d9`

`pnpm gate:s1` on `main`. Three measures MEASURED and green; four REFUSED.

| measure (new15 搂342 order) | tier | result |
|---|---|---|
| source completeness | MEASURED | 鉁?150/150 source records reached the served tree |
| artifact resolution rate | MEASURED | 鉁?149/150 resolvable; the 1 unresolvable is exactly the 1 the index marks `incomplete` |
| page quality | MEASURED | 鉁?149/149 baked subjects carry html+json+manifest and agree with the index on both digests |
| adapter failure rate | REFUSED | 鉁?0 attempts recorded |
| mean/p95 processing time | REFUSED | 鉁?no job has a recorded start/finish |
| CAS dedup rate | REFUSED | 鉁?45 blobs exist, `cas/manifests` = 0 鈥?no denominator (corrected 2026-08-28; first stated as `cas/blobs` = 0) |
| disk growth | REFUSED | 鉁?no baseline; needs two measurements over time |
...
> **CORRECTED 2026-08-28 鈥?the refusals were right and this paragraph's reason was false.** See
...
*not* the repo root 鈥?`pnpm --filter @calllint/trust-index 鈥 makes it the **package** directory.
So two stores exist, and the one the gate read is the empty one:

| | `.var/鈥 (repo root, what the gate read) | `packages/trust-index/.var/鈥 (what the worker writes) |
|---|---|---|
...
| `cas/blobs` | 0 | **45** (mtimes 2026-08-04 鈥?three weeks before this gate existed) |
| `compiler_jobs` / `compiler_runs` | 0 | **0** 鈫?the only two that were genuinely empty |
| `schema_migrations` | 2 | 3 |

The four measures remain `鉁?REFUSED`, and the load-bearing reason is unchanged 鈥?**a rate over an
...
45 blobs exist, `cas/manifests` holds 0, and the rate is blobs 梅 manifest references. Reporting
`45/0` as 100% would be the same empty-denominator defect **with a non-zero numerator** 鈥?the harder
...
- **source-completeness** is a true completeness claim: every entry in
  ...
  is **imported, not re-derived** 鈥?Workstream R measured raw-name joins at 0/19 and slug joins
...
|---|---|---|
| `pnpm gate:s1` | 0 | 3 MEASURED 鉁?/ 4 REFUSED 鉁?|
...
- **NC1** 鈥?drop one served entry 鈫?red **source-completeness** *and* the served floor. Two
  independent observers catch it, which is the only reason the floor is worth having next to a
  completeness check.
- **NC2** 鈥?flip one entry's status to `incomplete` 鈫?red **artifact-resolution** only, reported
  as `incomplete-but-resolvable` (the correct direction).
- **NC3** 鈥?corrupt one `pageDigest` 鈫?red **page-quality** only.

The index was restored after each and byte-identity confirmed with an empty `git status
--porcelain`.

### `--regression`, not `--gate`, in CI 鈥?and the reason is measured
...
would pin the required check red for a reason no PR under review can clear 鈥?verbatim the hazard that
...
is the reader 鈥?**33 `it` blocks, three layers**, modelled on the Gate S0 suite because the failure
modes are the same ones:

1. **POINTER TRUTH** 鈥?every `path:line` this file cites must resolve to a line *containing* what it
   ...
   of those onto docblock prose 鈥?a line that existed.

   > **THE POINTER LIST IS AN ENROLMENT, NOT A SCAN 鈥?measured 2026-09-01, and read this before adding
   ...
   > `runReport.ts:9999` 鈥?past the end of that file 鈥?and the suite stayed **32/32 green**. So a whole
   > section of unread citations can be added while the reader reports full coverage. Closed for that
   > section by enrolling all six (`the ADR 0097 clock path 鈥); **not** closed in general, because
...
2. **DERIVED-NOT-RESTATED** 鈥?every number here is recomputed from the file it describes. The census
...
3. **ROW STATUS + THE REFUSAL** 鈥?each row's `**Status:**` is asserted verbatim, each row must carry a
...
- **`ci.yml` is PARSED, not string-matched.** An unquoted `: ` inside a step name makes the whole
  ...
  red 鈥?a state every text assertion passes on. That is a measured failure from the S0 batch, not a
  hypothetical.
...
|---|---|---|
| NC1 | `refused("cas-dedup-rate", 鈥?` 鈫?`measured("cas-dedup-rate", true, 鈥?` | the refusal layer 鈥?*the defect this suite exists for* |
| NC2 | `ci.yml`'s step 鈫?`pnpm gate:s1:gate` | the parsed-wiring assertion |
| NC3 | S1-OPEN-2's `**Status:** **OPEN.**` 鈫?`**CLOSED.**` | the row-status layer |
| NC4 | one blank line prepended to `scripts/gate-s1.ts` | the pointer layer, naming what it found (`" */"`) |

Each mutation was reverted and byte-identity confirmed with `cmp` against a pre-mutation copy 鈥?not
...
assertion first reported **all 150** records missing from the served tree 鈥?the reverse of the
...
the namespace a second time, joining on `mcp-registry/mcp-registry/鈥. Importing the shipped function
is therefore **necessary but not sufficient** 鈥?mis-composing it produces the same 0/N join
...
## S1-OPEN-1 鈥?one of seven measures has no data source, so S1 cannot be CLAIMED passed

**Status:** **OPEN (narrowed three times: ~~four measures~~ 鈫?~~three~~ 鈫?~~two~~ 鈫?one, 2026-09-01).**

> **READ THE COUNT CAREFULLY 鈥?"one" here counts BLOCKERS, and `gate:s1` prints TWO refusals
...
> because renaming it would lose the distinction that took ADR 0097 to establish 鈥?but a reader
> comparing this heading against the gate's output will see 1 vs 2 and must not conclude the gate
> regressed.

Three green measures are not Gate S1. ~~The refused ones are precisely the *scale* measures 鈥?throughput, dedup efficiency, disk growth 鈥攡~ **re-aimed 2026-09-01:** that clause described the
refusals as a coherent *category*, and the category has since been dismantled from the inside 鈥?throughput (ADR 0097) and dedup efficiency (ADR 0093) are now measurable, and what remains is one
...
that scale. Those are different claims and S1 asks the second one 鈥?but the gap is now one measure
...
`adapter-failure-rate` acquired a real source 鈥?`refreshSnapshot.ts` now brackets every ingest in
`beginCompilerRun`/`concludeCompilerRun` and writes `reports/run-<id>.json`, which the gate reads 鈥?and the other three are each blocked by something *different*, which the single shared remedy hid.
...
reader had always assumed. The third closed because **the recorded blocker was false** 鈥?and it had
...
nobody an occasion to re-derive the reason underneath it. That is the ADR 0096 fault class 鈥?two
consecutive ADRs now 鈥?and it is the argument for reading a blocker's evidence when the row is
...
| `processing-time-mean-p95` | ~~**SCHEMA**~~ **RETRACTED 鈥?the blocker was false (ADR 0097)** | ~~`compiler_jobs` has `created_at`/`updated_at`/`available_at` and no `started_at`/`finished_at` (`migrations/001-canonical-adoption-graph.sql`), so no duration is recorded anywhere. Adding the columns breaks the 14-column 鈫?14-property equality `domain/job.ts:13` documents 鈫?needs a migration **and an ADR**. Running the compiler cannot unblock this.~~ **Every clause after the first was wrong.** `compiler_runs` has carried `started_at`/`completed_at` since the same canonical DDL, with a real writer 鈥?so "no duration is recorded anywhere" was false of the schema this row cites. `compiler_jobs` holds **0 rows** and `enqueueJobs` has no non-test caller, so the two proposed columns would have been NULL forever and the measure would have computed mean/p95 over an empty set: the empty-denominator defect, bought at the price of breaking a documented equality. And reading the columns that *do* exist closes nothing 鈥?all 3 rows have `completed_at 鈭?started_at = 0 ms`, because `refreshSnapshot.ts` passes one pinned `TRUST_INGEST_NOW` as both endpoints. **Now MEASURABLE** from a monotonic per-attempt clock in `resolveArtifacts.ts`, reported in `calllint.compiler-run-report.v3`. No migration was needed. **Read "MEASURABLE", not "MEASURED" 鈥?corrected 2026-09-01 in the fourth measurement, and the one-word difference is the whole of this row's remaining debt.** The writer ships and the whole path was re-read link by link, but the newest report on disk is still `v2`, so the gate REFUSES with `PREDATES THE OBSERVABLE`. This row said "Now MEASURED" while the gate printed `[REFUSED ]` 鈥?the same collapse of *has a source* into *has a measurement* that the second measurement was written to separate, reappearing one line above its own warning. |
...
the first commit, and nothing ever wrote one 鈥?the fourth instance in this store of the fault class
...
搂4 records the fork that was actually taken and the cheaper option that was rejected: summing
`verifyAndStore`'s `deduplicated` booleans into one counter would have closed the measure with one
integer and no new directory, and it cannot answer *against what* 鈥?nine requests for one blob and
nine distinct blobs sharing one prior both report "8 deduplicated" while meaning opposite things about
the store.

**Why the collapsed version was a defect and not just imprecision:** it named *one* remedy 鈥?"write
a queue driver" 鈥?for four blockers, three of which a queue driver does not touch. A reader who
...
controller/worker run against this checkout~~ 鈥?struck 2026-08-28, because the R-9 worker unit runs
three steps (`ingest:trust-index` 鈫?`project-adoption-index:trust-index` 鈫?`prune:cas`) and not one
...
on 2026-09-01: it is precisely what this row's own recorded remedy would have produced, and ADR 0097 搂D1
...
survive 鈥?and because **three of the four** now have sources, so the ways they can be circumvented have
...
- `processing-time-mean-p95`, which now *may* legitimately be MEASURED 鈥?and this clause is the
  ...
  鈥?struck 2026-09-01: `compiler_jobs` still carries no duration columns and never will, so as written
  ...
  fix. Re-aimed at what can now actually go wrong 鈥?if the gate prints a duration derived from
  `completedAt 鈭?startedAt` (0 ms by construction, since both are one pinned `TRUST_INGEST_NOW`), or
...
- `cas-dedup-rate` 鈥?the blocked-on-a-writer half of this clause is now **impossible**, and a
  ...
  a manifest's zero `references`, or collapses within-run reuse (`references 鈭?distinctDigests`) and
  ...
  `cas/manifests` is without a writer~~ 鈥?struck 2026-08-31, unreachable since ADR 0093.
- `disk-growth` 鈥?if it prints growth from one measurement, or from two taken without an intervening
  run.
...
(Stated against the specific missing artefacts, not against "`.var/` is empty" 鈥?the original
wording would have been *satisfied* by the mis-rooted read S1-OPEN-4 records.)

---

## S1-OPEN-2 鈥?S2 (500 records) has the same shape, one rung up, and no guard yet either

**Status:** **CLOSED 2026-08-31 鈥?the guard arrived before its threshold.**
...
Gates S1鈥揝4 as 猬? all in a gitignored file, and the cohort grows automatically under ADR 0086.
`S0_REGRESSION_FLOOR` is already **150**; the auto-growth adds +50/run toward a 500 ceiling. So
the cohort is on a path to cross **S2's 500-record threshold** the same way it crossed S1's 鈥?~~and today there is no `scripts/gate-s2.ts`, no `gate:s2` script, and no tracked artifact.~~
...
for one reason is not twice the coverage 鈥?it is one finding printed twice, and whoever fixes it
...
existing before the served cohort reaches 500 鈥?i.e. the guard arriving *before* its threshold,
...
`package.json`, this row failed at the one thing it was written to prevent* 鈥?and that can no longer
...
## S1-OPEN-3 鈥?the served-vs-committed floor is derived, and that is a deliberate divergence from S0
...
- S0 asks *did the committed cohort shrink between commits?* A literal is right there 鈥?only a
...
## S1-OPEN-4 鈥?this gate read the wrong store, and told every reader the compiler had never run
...
2026-08-28: the store the ingest worker actually writes, `packages/trust-index/.var/鈥, held a
**2551808-byte** database 鈥?298 canonical subjects, 1200 source records, 839 subject aliases, 78
...
`process.cwd()`. Under `pnpm --filter @calllint/trust-index 鈥 鈥?how every store-writing script is
invoked 鈥?that is the **package** directory. Under the systemd unit it is `WorkingDirectory=/opt/
...
a directory nothing writes to and report `inspected 0` forever" 鈥?in the tree *before* this gate
...
workflow)"* 鈥?and the workflow runs `pnpm ingest:trust-index`, i.e. `--filter`, i.e. the package
...
| 3 | `dirPopulated` counted the wrong unit | `readdirSync("cas/blobs").length` = **42** two-char shards for **45** blobs 鈥?and `paths.ts:117-118` explicitly warns callers "must not assume a shared traversal shape" |
...
12.9.0 for an ABI cliff, and a gate that cannot start is worse than one that reports less) 鈥?but for
...
- `storeCandidates` **discovers** rather than assumes: `ADOPTION_INDEX_CWD` (the seam `pruneCas.ts:59`
  and `backupAdoptionIndex.ts:52` already use 鈥?this gate must not invent a second convention), then
...
- The store census prints **in every mode, unconditionally** 鈥?including the passing ones. A census
...
`artifact_versions.artifact_status` carries a real distribution 鈥?**FETCHED 45 / RESOLVED 25 /
UNAVAILABLE 8** of 78 鈥?and `UNAVAILABLE` is adapter-shaped. It is deliberately **not** reported as
"adapter failure rate": that column grades an **artifact's** resolution state, not one **attempt's**
outcome, so 8/78 would answer a different question than 搂342 asks while wearing the name of the one
it asks. 搂342 wants attempts, which live in `compiler_jobs`, which needs the driver S1-OPEN-1 owes.

**What would close this half:** ~~either a queue driver (then attempts are the source, and this column
is irrelevant), or an ADR deciding that artifact status is an acceptable proxy~~ 鈥?**CLOSED 2026-08-31
...
mechanism while right about the requirement 鈥?recorded rather than corrected, because a remedy that
...
S1 will be publishing a number whose name misdescribes what it counted 鈥?the failure this row was
...
## Second measurement 鈥?2026-08-31, at `e40f9e3`

`pnpm gate:s1` on `fix/daily-approval-burden`. **Three MEASURED and green; four REFUSED 鈥?the same
...
| measure (new15 搂342 order) | tier | result | change since 2026-08-28 |
|---|---|---|---|
| source completeness | MEASURED | 鉁?150/150 source records reached the served tree | 鈥?|
| artifact resolution rate | MEASURED | 鉁?149/150 resolvable; the 1 unresolvable is exactly the 1 the index marks `incomplete` | 鈥?|
| page quality | MEASURED | 鉁?149/149 baked subjects carry html+json+manifest and agree with the index on both digests | 鈥?|
| adapter failure rate | REFUSED | 鉁?`no run report exists yet` | **source acquired.** Was "0 attempts recorded" (no source at all) |
| mean/p95 processing time | REFUSED | 鉁?blocked on SCHEMA 鈥?no `started_at`/`finished_at` columns | reason sharpened; blocker unchanged |
| CAS dedup rate | REFUSED | 鉁?45 blobs, `cas/manifests` = 0 鈥?`no CAS manifest exists` | **source acquired (ADR 0093).** Was MISSING WRITER: no format existed to write |
| disk growth | REFUSED | 鉁?baseline only | **baseline now recorded:** `packages/trust-index/.var/鈥 at 2551808 B, 45 blobs, 0 reports |
...
- *No source* 鈫?nothing could ever produce the number. That was the 2026-08-28 state.
- *Source, no data* 鈫?the writer exists, and no run has executed against this checkout since it
...
writer and no format existed. Both stores still print `cas/manifests=0` 鈥?the same number as before 鈥?which is exactly why the refusal text and not the census is where the change is legible. A gate whose
census is unchanged can still have moved.

The refusal names the second and points at `pnpm ingest:trust-index`, which is a **reachable** remedy 鈥?unlike the one struck from S1-OPEN-1 twice. That the number is still absent is therefore not a defect
...
**Verified through the real writer and the real gate, not by inspection.** The writer 鈫?JSON 鈫?gate
...
## S1-OPEN-5 鈥?`withCompilerRun` cannot record a crash in an async body, so a failed run stays RUNNING forever
...
- `jobStates.ts` gives `RUNNING` **no self-edge and no path out except through conclude**, so the row
  is unreachable forever 鈥?not stale, *stranded*.
...
bodies, and `refreshSnapshot.ts` **open-codes the bracket** rather than calling this helper 鈥?precisely
...
actually is 鈥?asserting `metricsRead === 0`, `state: "RUNNING"`, `completedAt: null` 鈥?with a comment
...
and concludes on rejection, plus the characterisation test inverted to assert the *correct* values 鈥?`FAILED`, a non-null `completedAt`, metrics read once.
...
## Third measurement 鈥?2026-09-01, at `7117aaa`+ (the first with a run report on disk)
...
came from performing the run the previous section declined to perform 鈥?plus fixing the defect that
would have made the run produce nothing anyway (ADR 0094).

| measure (new15 搂342 order) | tier | result | change since 2026-08-31 |
|---|---|---|---|
| source completeness | MEASURED | 鉁?200/200 source records reached the served tree | cohort 150 鈫?200 |
| artifact resolution rate | MEASURED | 鉁?199/200 resolvable; the 1 unresolvable is exactly the 1 the index marks `incomplete` | scaled with the cohort |
| page quality | MEASURED | 鉁?199/199 baked subjects carry html+json+manifest and agree with the index on both digests | scaled with the cohort |
| adapter failure rate | **MEASURED** | 鉁?0.0% 鈥?0/36 failed (0 unavailable + 0 rejected) over 36 fetched; 28 of 64 considered had no adapter and are excluded from BOTH halves | **REFUSED 鈫?MEASURED** |
| mean/p95 processing time | REFUSED | 鉁?blocked on SCHEMA 鈥?`compiler_jobs` has no `started_at`/`finished_at` | unchanged; needs a migration + an ADR |
| CAS dedup rate | **MEASURED** | 鉁?0.0% within-run reuse (0/36) and 0.0% already-on-disk (0/36), reported as two counts | **REFUSED 鈫?MEASURED** |
| disk growth | REFUSED | 鉁?baseline only, re-recorded at 185106432 B / 81 blobs / 1+ report | still one measurement; a rate needs two |
...
So `reports=2` and `cas/manifests=1` above are the **first such files in the project's history** 鈥?one FAILED report from the run that tripped a cohort refusal, one SUCCEEDED report with its manifest.
`adapter-failure-rate` and `cas-dedup-rate` are measured here for the first time ever.

**`disk-growth` still refuses, and that refusal is the honest one.** The baseline moved 2551808 鈫?185106432 B, which looks exactly like growth and is not: the earlier figure was read from a store
this gate was mis-rooted against (S1-OPEN-4), and 45 鈫?81 blobs spans a full mirror walk. Two numbers
...
`processing-time-mean-p95` 鈥?the migration it named was for a table holding zero rows with no writer,
...
made it read as settled. It is struck rather than deleted for exactly that reason 鈥?the sequence of
...
## Fourth measurement 鈥?2026-09-01, at `c4c388b`+ (the ADR 0097 writer, not yet exercised)

`pnpm gate:s1` on `feat/new22-cursor-authority-boundary-v2`, EXIT **0**. **Five MEASURED and green;
two REFUSED** 鈥?the same counts as the third measurement. The change is again in a refusal's *reason*,
...
| measure (new15 搂342 order) | tier | result | change since third measurement |
|---|---|---|---|
| source completeness | MEASURED | 鉁?200/200 | 鈥?|
| artifact resolution rate | MEASURED | 鉁?199/200; the 1 unresolvable is exactly the 1 the index marks `incomplete` | 鈥?|
| page quality | MEASURED | 鉁?199/199 agree on both digests | 鈥?|
| adapter failure rate | MEASURED | 鉁?0.0% 鈥?0/36 over 36 fetched; 28 of 64 considered excluded from BOTH halves | 鈥?|
| mean/p95 processing time | REFUSED | 鉁?**`PREDATES THE OBSERVABLE`** 鈥?the newest report on disk is `calllint.compiler-run-report.v2` | **reason changed; blocker is now a run, not a schema** |
| CAS dedup rate | MEASURED | 鉁?0.0% within-run reuse (0/36) and 0.0% already-on-disk (0/36), as two counts | 鈥?|
| disk growth | REFUSED | 鉁?baseline only 鈥?185106432 B, 81 blobs, 1+ report | unchanged: still one measurement |
...
Verified as a path, not inferred from the message 鈥?each link read at this commit:
...
So the blocker is now **one ingest**, and this measurement deliberately did not perform it 鈥?the same
...
which is **tracked** (sha256 `6aa729c3鈥 at this commit). Only the first kind is what this measure needs.
A measurement pass that rewrote a committed snapshot and advanced Gate S0's ratchet as a side effect
would be trading a tracked artifact for a number 鈥?so the run is left as an explicit act, not a side
effect of reading a gate.

**`disk-growth` is unchanged and remains the one refusal no run closes.** Its baseline is re-read here
(185106432 B / 81 blobs), identical to the third measurement's, because nothing has run in between 鈥?which is itself the demonstration: two readings of one state are not two measurements, and the gate
...
`pnpm gate:s2` EXIT **0** 鈥?4 MEASURED / 1 REFUSED, cohort **200 / 500**. The refusal is worth quoting
...
Upstream is not the constraint 鈥?86984 records were read. The constraint is the **snapshot cap**, and
the gate names two distinct remedies: let auto-growth run (+50/run 鈬?~6 ingests), or raise
...
**Both of those remedies are gated by ADR 0009, which the gate's message does not mention** 鈥?checked
...
output size. Its argument is a distinction this file's own measures keep running into 鈥?*"a single-shot
fetch that works at 100 may exceed a platform timeout at 500; that is a cliff, not a slope, and it
cannot be extrapolated from one data point"* 鈥?and it flags mirror read volume as the quantity that
*"turns into someone else's rate limit 鈥?an external harm, not a local slowdown."* Auto-growth is
...
So S2's blocker is **absent instrumentation**, not elapsed time and not a missing env var 鈥?and the
instrumentation is a measurement task (two cohort sizes 脳 three quantities) that nothing in this repo
...
inside a Gate S1 measurement pass is how an unmeasured 5脳 expansion of someone else's rate limit gets
