...
### Verified result (0.3.0-preview.1, 2026-06-17)
...
The attestation is a SLSA v1 provenance predicate
(`predicateType: https://slsa.dev/provenance/v1`), generated by the
Trusted-Publishing (OIDC) release workflow — no stored token.

### Gotcha: a mirror registry breaks signature verification

`npm audit signatures` fetches npm's TUF signing keys from the **official**
registry. If the machine's `registry` is set to a mirror
(e.g. `https://registry.npmmirror.com/`), key fetch fails and npm reports a
misleading `EMISSINGSIGNATUREKEY` — *the artifact is fine; the local registry is
the problem*. Always pin the official registry when verifying:
...
`npm@latest` is used because signature/attestation verification needs npm
≥ 11.5.1; older CLIs (e.g. the Node 20 bundled npm 10.x) lack the newer key.
...
The same facts must hold across npm, GitHub, and the website. Verified for
`0.3.0-preview.1` on 2026-06-17:

- [x] npm version == git tag (`v<version>`) == `apps/cli/package.json` version.
- [x] npm `repository`/`homepage` point at `calllint/calllint` and calllint.com.
- [x] The published tarball contains only the `files` allowlist (`dist`,
      `LICENSE`, `NOTICE`, `logo-mark-128.png`) — no tests, corpus, or
      `.github`.
- [x] A GitHub Release exists for the tag with notes from `docs/releases/`
      (`v0.3.0-preview.1`, marked pre-release, includes Known limitations).
