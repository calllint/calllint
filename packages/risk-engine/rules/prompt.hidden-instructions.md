...
Scope (ADR 0014): reads the config's declared tool metadata only. README /
SKILL.md / package description / registry metadata are out of scope for v0 (they
need new input plumbing) and are the remaining R4 work. Static shape detection
only — it does not claim to detect prompt injection or infer intent.
